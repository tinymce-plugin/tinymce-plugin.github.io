 ...
 */
 ...
 (function (domtoimage) {
    ...
    /* Five 2021-05-06 start*/ 
    var defaultTableToolbar = 'tableinsertrowbefore tableinsertrowafter tabledeleterow | tableinsertcolbefore tableinsertcolafter tabledeletecol | tablealignleft tablealigncenter tablealignright tabletoimg tableprops tabledelete';
      /* Five 2021-05-06 end*/ 
    var defaultStyles = {
  ...
  /* Five 2021-05-06  register Commands start*/ 
      var leftTable = function () {
        getSelectionStartCellOrCaption$1(editor).each(function (cellOrCaption) {
          table(cellOrCaption, isRoot).filter(not(isRoot)).each(function (table) {
            var cursor = SugarElement.fromText('');
            after(table, cursor);
            editor.formatter.apply('alignleft', {}, table.dom) ;
            if (editor.dom.isEmpty(editor.getBody())) {
              editor.setContent('');
              editor.selection.setCursorLocation();
            } else {
      ...
      var rightTable = function () {
        getSelectionStartCellOrCaption$1(editor).each(function (cellOrCaption) {
          table(cellOrCaption, isRoot).filter(not(isRoot)).each(function (table) {
            var cursor = SugarElement.fromText('');
            after(table, cursor);
            editor.formatter.apply('alignright', {}, table.dom) ;
            if (editor.dom.isEmpty(editor.getBody())) {
              editor.setContent('');
              editor.selection.setCursorLocation();
            } else {
      ...
      var centerTable = function () {
        getSelectionStartCellOrCaption$1(editor).each(function (cellOrCaption) {
          table(cellOrCaption, isRoot).filter(not(isRoot)).each(function (table) {
            var cursor = SugarElement.fromText('');;
            after(table, cursor);
            editor.dom.setStyle(table.dom, 'float','none');
            editor.formatter.apply('aligncenter', {}, table.dom);
            if (editor.dom.isEmpty(editor.getBody())) {
              editor.setContent('');
              editor.selection.setCursorLocation();
            } else {
          ...
          table(cellOrCaption, isRoot).filter(not(isRoot)).each(function (table) {
           ...
           console.log(table)
            ...
            .then(function (theBlob) {
               ...
               let images_upload = editor.getParam('images_upload_handler', undefined, 'function');
               theBlob.lastModifiedDate = new Date();
               theBlob.name = 'toimg'+new Date().getTime()+'.png';
               let blobInfo = {
                    file: null,
                    blob: function(){
                    return theBlob
               ...
               images_upload(blobInfo,function(url){
                    remove$2(table);
                    if (editor.dom.isEmpty(editor.getBody())) {
                      editor.setContent('');
                      editor.selection.setCursorLocation();
...
/* Five 2021-05-06  register Commands end*/ 
...
/* Five 2021-05-06  addCommand  start*/ 
        mceTableAlignLeft: leftTable,
        mceTableAlignCenter: centerTable,
        mceTableAlignRight: rightTable,
        mceTableToImg: tableToImg,
/* Five 2021-05-06  end*/ 
      ...
      editor.addCommand('mceTpable', function (_ui, args) {
        if (isObject(args) && keys(args).length > 0) {
          insertTableWithDataValidation(editor, args.rows, args.columns, args.options, 'Invalid values for mceTpable - rows and columns values are required to insert a table.');
        } else {
          open$2(editor, true);
        }
      });
    ...
    var addButtons = function (editor, selectionTargets, clipboard) {
      editor.ui.registry.addMenuButton('tpTable', {
        tooltip: 'Table',
        icon: 'table',
...
/* Five 2021-05-06 add Button  start*/ 
      editor.ui.registry.addButton('tablealignleft', {
        tooltip: '表格居左',
        onAction: cmd('mceTableAlignLeft'),
        icon: 'align-left-table',
        onSetup: selectionTargets.onSetupTable
      });
      editor.ui.registry.addButton('tablealigncenter', {
        tooltip: '表格居中',
        onAction: cmd('mceTableAlignCenter'),
        icon: 'align-center-table',
        onSetup: selectionTargets.onSetupTable
      });
      editor.ui.registry.addButton('tableToimg', {
        tooltip: '表格转为图片',
        onAction: cmd('mceTableToImg'),
        icon: 'table-to-img',
        onSetup: selectionTargets.onSetupTable
      });
      editor.ui.registry.addButton('tablealignright', {
        tooltip: '表格居右',
        onAction: cmd('mceTableAlignRight'),
...
/* Five 2021-05-06 end*/ 
    ...
    var addToolbars = function (editor) {
      ...
      if (toolbar.length > 0) {
        editor.ui.registry.addContextToolbar('tpTable', {
          predicate: isTable,
          items: toolbar,
          scope: 'node',
          position: 'node'
    ...
    var addMenuItems = function (editor, selectionTargets, clipboard) {
      ...
      var insertTableAction = function (data) {
        editor.execCommand('mceTpable', false, {
          rows: data.numRows,
...
/* Five 2021-05-06  end*/ 
      ...
      editor.ui.registry.addContextMenu('tpTable', {
        update: function () {
