 ...
 (function () {
  ...
  var applyListFormat = function (editor, listName, styleValue) {
     let listElm = editor.dom.getParent(editor.selection.getNode(), 'ol,ul');
    let _class = listElm ? editor.dom.getAttrib(listElm, 'class').replace(/tp-iconlists(_[A-Za-z0-9]+)/g,'').replace(/tp-iconlists/g,'') : ''
    let _value = styleValue === false ? null : { 'list-style-type': styleValue,"list-attributes": { class: 'tp-iconlists_default'+ _class } }
    typeof styleValue === 'string' && styleValue.match(/^tp-/) && (_value = {
      'list-style-type': styleValue,
      "list-attributes": {
        class: _class +" tp-iconlists " + styleValue
      }
    })
  ...
  var getBulletStyles = function (editor) {
    var styles = editor.getParam('tp_iconlists_styles', 'default,circle,square,tp-iconlists_tick,tp-iconlists_layout');
  ...
  var register$1 = function (editor) {
    ...
    addControl(editor, 'tpIconlists', 'Icon list', 'InsertUnorderedList', 'UL', getBulletStyles(editor));
  };

  function Plugin () {
    global.add('tpIconlists', function (editor) {
