var Ft=(c,b)=>()=>(b||c((b={exports:{}}).exports,b),b.exports);var Bt=Ft((Tt,z)=>{/*! copy
...
*/(function(){let c=tinymce,b=c.util.Tools,y=c.html.Node;c.html.Schema;let q=c.util.XHR,S=c.util.I18n,G=new c.html.Serializer().serialize,X=new c.html.DomParser().parse,x={};window.tp$State=x;let h=(t,a)=>{let n=a?a.match(new RegExp(t+':(.+?)"?[;}]')):"";return n?n[1]:!1};function D(t,a,n,e){t.tp$Style.mapping&&t.tp$Style.mapping[""+a.getAttribute("data-id")]&&(t.tp$Style.mapping[""+a.getAttribute("data-id")].specialStyle[""+n]=e)}let g={customTags:{}},_=Object.keys,m=function(){return m=Object.assign||function(a){for(var n,e=1,i=arguments.length;e<i;e++){n=arguments[e];for(var o in n)Object.prototype.hasOwnProperty.call(n,o)&&(a[o]=n[o])}return a},m.apply(this,arguments)},Y=Object.hasOwnProperty;var Z=function(t){return t==null},Q=function(t){return!Z(t)};const v=t=>{if(typeof t=="object")return JSON.stringify(t).replace(/"([-A-Za-z]+?)":""[,}]/g,"").replace(/,/gi,";").replace(/{/gi,"").replace(/}/gi,"").replace(/"/gi,"")},A=t=>typeof t=="string"&&t!=="{}"?(t=JSON.stringify(t),JSON.parse(("{"+t.replace(/:/g,'": "').replace(/;\s*/g,'","')+"}").replace(/,\"\"\}$/,"}"))):t,k=(t,a)=>{const n=a.match(new RegExp(`.${t}\\s*\\{([\\s\\S]+)\\}`));return n&&n[1]?n[1].replace(/\}([\s\S]+)/,"").trim():""};var tt=function(t,a){return Y.call(t,a)},et=function(t,a,n,e){e===void 0&&(e=null);var i=t.attr(n);return Q(i)?i:tt(a,n)?null:e};const L=t=>t.replace(/[-|\_](\w)/g,function(a,n){return n.toUpperCase()}),at=t=>t.replace(/([A-Z])/g,"_$1").toLowerCase().replace(/\-/g,"_"),nt=t=>t.replace(/([A-Z])/g,"-$1").toLowerCase().replace(/\_/g,"-");var it=function(t,a){let n="";if(!t)return"";for(a=L(a);t.nodeName!=="P"&&t.nodeName!=="LI"&&t.nodeName!=="DIV"&&t.nodeName!=="BODY";){if(t.style[a]){n=t.style[a];break}t=t.parentNode}return n};let E=(t,a)=>{let n="";return n+=t["tp-buttons"]?'.tp-buttons[data-tp-style="'+a+'"] {'+t["tp-buttons"]+`}
...
`:"",n};const ot=(t,a,n)=>{!n&&(n=a*100);let e={id:null,outTime:n,outId:null};e.id=setInterval(i=>{t(()=>{clearTimeout(i.outId),clearInterval(i.id)})},a,e),e.outId=setTimeout(()=>{e.id&&clearInterval(e.id)},e.outTime)};let C=function(t,a){return typeof t[a]=="function"?t[a]:typeof g.customTags[t.name][a]=="function"?g.customTags[t.name][a]:function(){}};const st=(t,a)=>a?typeof t=="string"&&!t.match(/\s/)&&t.length>0?parseInt(t)+"px":t:typeof t=="string"&&t.length>0&&t.match(/^[0-9]{1,8}$/)?t+"px":t,H=t=>{let a=t.split("_");return a.length>1?S.translate([a[0]+" {0}",a[1]]):S.translate(t)};tinymce.tp$HtmlPanelFn=window.tp$HtmlPanelFn=function(t,a,n){x.buttonsStyle&&(x.buttonsStyle[a]=n),document.querySelector("#"+a+"_StyleID").innerHTML=t.nextElementSibling.innerHTML};const lt={count:0},rt=t=>{let a=new Date().getTime()+"-"+lt.count++,n=`
...
<div class="tp-collapse_mainBox">
  ...
  .skt-loading .skeleton.badge:after {
  background-color: #f8fafc
  }`,mt=t=>{if(!document.querySelector("style#skt-tox-style")){let s=document.createElement("style");s.type="text/css",s.id="skt-tox-style";try{s.appendChild(document.createTextNode(U()))}catch{s.styleSheet.cssText=U()}document.getElementsByTagName("head")[0].appendChild(s)}if(!document.querySelector("style#skt-dark-tox-style")){let s=document.createElement("style");s.type="text/css",s.id="skt-dark-tox-style";try{s.appendChild(document.createTextNode(J()))}catch{s.styleSheet.cssText=J()}document.getElementsByTagName("head")[0].appendChild(s)}let a=[];typeof t.toolbar=="string"?a=j(t.toolbar):Array.isArray(t.toolbar)&&t.toolbar.forEach(s=>{a.push(...j(s))});let n=[];if(t.menubar!==!1)if(typeof t.menubar=="string")n=gt(t.menubar);else for(let s in w.title)O[s]&&n.push(w.title[s][t.language||"en_US"]);let e=t.min_height||t.height||200,i=[],o=e-150;for(let s=0;s<o;s+=50)i.push("1");let l=t.selector?document.querySelector(t.selector).parentNode:t.target.parentNode,r=document.createElement("div");return r.className="skt skt-loading",/dark$/.test(t.skin)&&r.setAttribute("data-isdark","true"),r.innerHTML=yt({selector:t.selector,toolbar:a,menubar:n,branding:t.branding!==!1,placeholderList:i,height:e}),l.style.position="relative",l.style.minHeight=e+"px",l.append(r),r},bt=t=>{let a="";return t.forEach(n=>{typeof n=="object"?n.forEach(e=>{a+='<button  class="skt-tox-mbtn skt-tox-mbtn--select"><span class="skt-tox-mbtn__select-label skeleton">'+e+`</span></button>
`}):a+='<button  class="skt-tox-mbtn skt-tox-mbtn--select"><span class="skt-tox-mbtn__select-label skeleton">'+n+`</span></button>
`}),a},xt=t=>{let a="";return t.forEach(n=>{a+=`<div class="skt-tox-toolbar__group">
...
`}),a+=`</div>
`}),a},ft=t=>{let a="";return t.forEach(n=>{a+=`<p class="skeleton"></p>
`}),a},yt=t=>`
...
</div>`;g.customTags[M.name]=M,g.customTags[$.name]=$,g.customTags[V.name]=V;let Ct=(t,a,n)=>{let e=`
...
}`:"")+e+i+o+n+p};const Et={iframeLayout:`
   <style>
